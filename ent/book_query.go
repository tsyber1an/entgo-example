...
func (bq *BookQuery) QueryAuthor() *PersonQuery {
	query := &PersonQuery{config: bq.config}
...
func (bq *BookQuery) WithAuthor(opts ...func(*PersonQuery)) *BookQuery {
	query := &PersonQuery{config: bq.config}
...
func (bq *BookQuery) sqlAll(ctx context.Context) ([]*Book, error) {
	...
	if query := bq.withAuthor; query != nil {
		...
		for i := range nodes {
			if nodes[i].person_books == nil {
				continue
			}
			fk := *nodes[i].person_books
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "person_books" returned %v`, n.ID)
			}
			for i := range nodes {
