...
func (pq *PersonQuery) QueryBooks() *BookQuery {
	query := &BookQuery{config: pq.config}
...
func (pq *PersonQuery) sqlAll(ctx context.Context) ([]*Person, error) {
	...
	if query := pq.withBooks; query != nil {
		...
		query.Where(predicate.Book(func(s *sql.Selector) {
			s.Where(sql.InValues(person.BooksColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.person_books
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "person_books" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "person_books" returned %v for node %v`, *fk, n.ID)
			}
